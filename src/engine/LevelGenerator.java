...
public class LevelGenerator implements Runnable {
    ...
    public static String folder = "." + File.separator + "data" + File.separator + "levels" + File.separator + "";
    ...
    public LevelGenerator(String lvName, JProgressBar progressbar, JLabel statusLabel) {
    ...
    public void run() {
        ...
        LevelGenerator.graphConfigSetup();
        LevelGenerator.puzzleConfigSetup();
        ...
        if(!finished){
            ...
            baseStatus = "Exporting Level";
            exportGraph(g);
            baseStatus = "Level Generated.";
        }
        else{
    ...
    private static void puzzleConfigSetup() {
    ...
    private static void graphConfigSetup() {
    ...
    public void exportGraph(Graph g){
        ...
        if(levelName.isEmpty()){
            levelName = ""+g.hashCode();
        }
        
        File dir = new File(LevelGenerator.folder, levelName);
        if(!dir.exists()){
            dir.mkdir();
        }
        else{
        ...
        LevelFileWriter fw = new LevelFileWriter(g, levelName);
        System.out.println("Exporting Map\n"+
                File.separator+"data_"+levelName+".json\n"+
                File.separator+"map_"+levelName+".json");
        fw.exportDataJSON(dir+File.separator+"data_"+levelName+".json",
                "GraphStream", maxX, maxY, false);
        fw.exportMapJSON(dir+File.separator+"map_"+levelName+".json",
                "Evolutionary"+BarabasiAlbertGenerator.class.getSimpleName(), false);
        
        // export PNG
        GraphImageBuilder gib = new GraphImageBuilder(dir+File.separator+"img_"+levelName+".png");
        gib.buildImage(g);
    }
}
